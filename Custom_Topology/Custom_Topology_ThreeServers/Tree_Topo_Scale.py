...
def main():
    ...
    subprocess.run(["touch", "S.txt","S1.txt","S2.txt","H.txt","K.txt","N.txt","O.txt"])

    S.cmd('iperf -s |& tee -a S.txt &')
    S1.cmd('iperf -s |& tee -a S1.txt &')
    S2.cmd('iperf -s |& tee -a S2.txt &')

    sleep(10)
    print(H.cmd('iperf -c {} |& tee -a H.txt &'.format(S1.IP())))
